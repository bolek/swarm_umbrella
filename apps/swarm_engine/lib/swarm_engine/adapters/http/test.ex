defmodule SwarmEngine.Adapters.HTTP.Test do
  def request(term, url, headers, body, opts) do
    with {:ok, _} <- validate_http_request(url),
         content <- gen_content(term, url, headers, body, opts),
         {:ok, pid} <- Agent.start_link(fn -> content end) do
  ...
  defp gen_content(_term, _url, _headers, _body, opts) do
    ["requested", "data"]
  end
end
